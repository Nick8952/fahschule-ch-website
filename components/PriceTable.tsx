...
export default async function PriceTable() {
  ...
  return (
    <div>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="border-b border-ink/10 last:border-0 hover:bg-chalk-2/60">
                <td className="px-4 py-3.5 text-[0.94rem] text-ink-soft"><TranslatedText de={r.label} /></td>
                <td
      ...
      <p className="mt-3 text-[0.84rem] text-ink-soft/70"><TranslatedText de={prices.disclaimer} /></p>
    </div>
  );
}
