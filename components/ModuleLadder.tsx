...
export default async function ModuleLadder() {
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)]">
      <div>
        <p className="eyebrow mb-4"><TranslatedText de="Unterrichtskonzept" /></p>
        <h2 className="max-w-[16ch] text-step-3 font-extrabold text-white"><TranslatedText de={modules.heading} /></h2>
        <p className="mt-3 max-w-[44ch]"><TranslatedText de={modules.intro} /></p>
        <div className="mt-6 flex items-center gap-3 font-mono text-[0.74rem] uppercase tracking-[0.1em] text-on-dark-faint">
          <span><TranslatedText de="Taxonomie" /></span>
        ...
        {items.map((m, i) => (
          <RevealLi key={m.range} delay={i * 70} className="relative pb-8 last:pb-0">
            ...
            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
              <span className="font-mono text-[0.78rem] text-signal-soft"><TranslatedText de="Modul" /> {m.range}</span>
              <span className="border border-steel px-1.5 py-0.5 font-mono text-[0.7rem] text-on-dark-soft">
                <TranslatedText de="Stufe" /> {m.level}
              </span>
            </div>
            <h3 className="mt-1.5 font-display text-step-1 font-bold text-white"><TranslatedText de={m.title} /></h3>
            <p className="mt-1 text-[0.93rem]"><TranslatedText de={m.body} /></p>
          </RevealLi>
        ))}
      </ol>
    </div>
